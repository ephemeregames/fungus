(function() {
  var Canvas2D, Canvas3D, Game, Launcher, Random, Scene, Scenes, Text;
  ...
  this.Fatal = (function() {
    function Fatal() {
      ...
      this.setPlatformUnsupported = __bind(this.setPlatformUnsupported, this);      $('window').bind('resize', __bind(function() {
        return this.resize();
      }, this));
    }
    Fatal.prototype.setPlatformUnsupported = function() {
      $('body').append('<div id="fungusError">Platform unsupported.</div>');
      return $('fungusError').center();
    };
    Fatal.prototype.resize = function() {
      var element;
      element = $('fungusError');
      if (!(element != null)) {
        return;
  ...
  Canvas3D = (function() {
    ...
    Canvas3D.prototype.resize = function(size) {
      this.size = size;
      this.renderer.setSize(this.size.x, this.size.y);
      return fullscreen();
    };
    Canvas3D.prototype.fullscreen = function() {
      var e, w;
      e = $(this.renderer.domElement);
      w = $('window');
      e.css('width', w.width());
      return e.css('height', w.height());
    };
    return Canvas3D;
  })();
  Scene = (function() {
  ...
  Scenes = (function() {
    function Scenes() {
      this.resize = __bind(this.resize, this);
      this.draw = __bind(this.draw, this);
      this.setBufferSize = __bind(this.setBufferSize, this);
      this.setUseFixedDrawingBuffer = __bind(this.setUseFixedDrawingBuffer, this);
      this.setActive = __bind(this.setActive, this);
      this.createScene = __bind(this.createScene, this);      this._bufferSize = {
        x: $(window).width(),
        y: $(window).height()
      };
      this._fixedDrawingBuffer = false;
      this.canvas = new Canvas3D(this._bufferSize);
    ...
    Scenes.prototype.setUseFixedDrawingBuffer = function(use) {
      return this._fixedDrawingBuffer = use;
    };
    Scenes.prototype.setBufferSize = function(size) {
    ...
    Scenes.prototype.resize = function() {
      ...
      for (name in _ref) {
        scene = _ref[name];
        scene.setSize(this._bufferSize);
      }
      this.canvas.resize(this._bufferSize);
  ...
  Game = (function() {
    function Game() {
      ...
      this.draw = __bind(this.draw, this);
      this.update = __bind(this.update, this);      this._timerVisual = new Timer(60);
  ...
  Launcher = (function() {
    function Launcher() {
      this._fatal = new Fatal();
      try {
        this.game = new window.Game();
        this.game.start();
      } catch (error) {
        if (error === "PlatformUnsupported") {
          this._fatal.setPlatformUnsupported();
        }
        if ((this.game != null)) {
          this.game.stop();
        }
